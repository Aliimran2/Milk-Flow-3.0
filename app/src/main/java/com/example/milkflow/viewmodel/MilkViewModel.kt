...
class MilkViewModel(private val repository: MilkRepository) : ViewModel() {
    ...
    fun updateCollectorTotal(collectors: List<PersonModel>) {
        _totalCollectorAmount.value = collectors.sumOf { it.personRate * it.personQuantity }
    }
    ...
    fun calculateDifference() {
        _difference.value =  (_totalAmount.value
